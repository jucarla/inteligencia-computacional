...
class SmartBatteryPlayer(BasePlayer):
    ...
    def __init__(self, position):
        ...
        self.base_battery_threshold = 25  # Limiar base para recarregar
    ...
    def escolher_alvo(self, world):
        ...
        # Se estiver carregando um pacote
        if self.cargo > 0:
            if world.goals:
                best_goal = None
                ...
                for goal in world.goals:
                    value = self.calculate_delivery_value(current_pos, goal, world.recharger)
                    if value > best_value:
                        # Verifica se tem bateria suficiente para a entrega
                        delivery_dist = self.distance_to(current_pos, goal)
                        if self.battery >= delivery_dist or delivery_dist <= 5:  # Permite entregas próximas mesmo com bateria baixa
                            best_value = value
                            best_goal = goal
            ...
            # Se não encontrou entrega viável e bateria está baixa, recarrega
            if self.battery < battery_threshold:
                return world.recharger

        # Se não estiver carregando e houver pacotes
        elif world.packages:
            best_pkg = None
            best_value = float('-inf')
            
            for pkg in world.packages:
                # Calcula valor considerando entrega mais próxima após pegar o pacote
                pkg_dist = self.distance_to(current_pos, pkg)
                if self.battery >= pkg_dist or pkg_dist <= 5:  # Permite pegar pacotes próximos mesmo com bateria baixa
                    # Encontra a entrega mais próxima deste pacote
                    closest_goal = None
                    ...
                    if closest_goal:
                        value = self.calculate_delivery_value(pkg, closest_goal, world.recharger)
                        if value > best_value:
                            best_value = value
                            best_pkg = pkg
            ...
            # Se não encontrou pacote viável e bateria está baixa, recarrega
            if self.battery < battery_threshold:
                return world.recharger

        # Se bateria está muito baixa e não há ações viáveis, recarrega
...
class Maze:
    ...
    def game_loop(self):
        ...
        if self.record:
            ...
            video_filename = f"{video_dir}/game_recording_v1_{timestamp}.gif"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Peso do jogador (influencia o algoritmo de decisão)."
    )
    parser.add_argument(
        "--delay",
        type=int,
    ...
    parser.add_argument(
        "--record",
        action="store_true",
        help="Grava o vídeo do jogo (salva como GIF)."
    )
    args = parser.parse_args()
    
    maze = Maze(seed=args.seed, delay=args.delay, record=args.record)
    
    # Ajusta o peso do jogador (se for SmartBatteryPlayer)
    if isinstance(maze.world.player, SmartBatteryPlayer):
        maze.world.player.base_battery_threshold = 25 * args.weight
        maze.world.player.base_min_battery = 15 * args.weight
